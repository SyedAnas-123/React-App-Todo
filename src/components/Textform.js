...
export default function Textform(props) {
    ...
    const clearup=() =>{
     ...
     const handleExtraSpaces=()=>{          //used regix in java script
...
// now all work of changind dark mode and all text into whoite color done in this component
  return (
    <>
    <div className="container"style={{color: props.mode === 'dark'?'white':'#091e3d'}} >
      ...
      <div className="mb-3">
        ...
        <textarea
          ...
          style={{backgroundColor: props.mode === 'dark'?'#000':'grey' , color: props.mode === 'dark'? 'white':'#091e3d'}}
      ...
      <button type="button" className="btn btn-primary mx-2"onClick={handleUpClick}>Convert to uppercase
  </button>


  <button type="button" className="btn btn-primary mx-2"onClick={clearup}>Undo all</button>
  <button type="button" className="btn btn-primary mx-2"onClick={handleExtraSpaces}> Remove Extra Spaces</button>
    ...
    <div className="conatiner my-5" style={{color: props.mode === 'dark'?'white':'#091e3d'}} >
        <h2>Your Text Summary</h2>

{/* logic for counting no of words in text box and removinf the default string which creates words = 1   */}

        <p>{text.split(" ").length >1 ?text.split(" ").length-1 : 0 } words and {text.length} charahters</p>
